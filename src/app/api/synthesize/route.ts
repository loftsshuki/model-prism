...
export async function POST(req: NextRequest) {
  ...
  try {
    if (input.runId) {
      const run = await getRun(input.runId, await requestOwner(req));
      if (!run) return NextResponse.json({ error: "Run not found" }, { status: 404 });
      if (run.snapshot?.background) return NextResponse.json({ error: "Use Resume or Second pass to update a background review" }, { status: 409 });
    }

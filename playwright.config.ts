...
export default defineConfig({
  ...
  use: { baseURL: process.env.PRISM_TEST_URL ?? "http://localhost:3107", storageState: process.env.PRISM_TEST_STORAGE_STATE, headless: true, trace: "retain-on-failure",
  ...
  webServer: process.env.PRISM_TEST_URL ? undefined : { command: "npm run start -- --hostname localhost --port 3107", url: "http://localhost:3107", reuseExistingServer: !process.env.CI, timeout: 60000 },
});
